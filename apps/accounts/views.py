...
class ChangePasswordView(APIView):
    """Allow an authenticated user (via access_token cookie or Authorization header)
    to change their own password.

    Expects JSON: { "old_password": "...", "new_password": "...", "confirm_password": "..." }

    Only the user identified by the token may change their password. The view
    will attempt to authenticate using the `access_token` cookie when
    `request.user` is not already authenticated.
    """
    ...
    def post(self, request):
        ...
        # If not authenticated via standard mechanisms, try cookie-based token
        if not (user and getattr(user, 'is_authenticated', False)):
            token = request.COOKIES.get('access_token')
            if token:
                request.META['HTTP_AUTHORIZATION'] = f'Bearer {token}'
        ...
        if not all([old_password, new_password, confirm_password]):
            return Response({"detail": "old_password, new_password and confirm_password are required"}, status=status.HTTP_400_BAD_REQUEST)

        if new_password != confirm_password:
            return Response({"detail": "new_password and confirm_password do not match"}, status=status.HTTP_400_BAD_REQUEST)

        try:
            if not user.check_password(old_password):
                return Response({"detail": "old_password is incorrect"}, status=status.HTTP_400_BAD_REQUEST)
        except Exception:
            # If check_password is not available for some reason, deny
            logger.exception("Password check failed for user id=%s", getattr(user, 'id', None))
            return Response({"detail": "Unable to verify password"}, status=status.HTTP_400_BAD_REQUEST)

        # All good: set the new password (handles hashing)
        try:
            user.set_password(new_password)
            # Save the user; update_fields may not include modified_at auto field,
            # so do a full save for legacy unmanaged model.
            user.save()
            logger.info("Password changed for user id=%s", getattr(user, 'id', None))
            return Response({"message": "Password changed successfully"}, status=status.HTTP_200_OK)
        except Exception:
            logger.exception("Failed to update password for user id=%s", getattr(user, 'id', None))
